...
const Statistics = () => {
  ...
  const fetchPercentages = async () => {
    try {
      ...
      const response = await fetch(
        `https://fresh-track-api.onrender.com/api/items/stats/category-percentages`,
        {
          method: 'GET',
          headers: {
      ...
      if (!response.ok) {
        throw new Error(`Category percentages fetch failed: ${response.statusText}`);
      }
      const data = await response.json();
      console.log(data);

      // if there's no data (or is empty) use default categories
      if (data.length === 0) return;

      const newCategories = {};
      data.forEach(({ category, percentage }) => {
        newCategories[category] = percentage;
      });

      setData(previousData => ({ ...previousData, categories: newCategories }));
    } catch (error) {
  ...
  const fetchExpiryData = async () => {
    try {
      setLoading(true);
      const response = await fetch(
        `https://fresh-track-api.onrender.com/api/items/stats/most-frequent-expired`,
        {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
      ...
      if (!response.ok) {
        throw new Error(`Expiry Data fetch failed: ${response.statusText}`);
      }
      const data = await response.json();
      console.log(data);
    } catch (error) {
